class root_finding:
    ...
    def n_bisection(self, f, xL, xR, n=100, tolerance=1e-12):
        ...
        for interval in sub_intvls:
            if f(interval[0])*f(interval[1]) < 0:
                roots.append(self.bisection(
                    f, interval[0], interval[1], tolerance))
        return roots
    ...
    def n_hybrid(self, f, xL, xR, df=None, n=100, tolerance=1e-12):
        ...
        for interval in sub_intvls:
            if f(interval[0])*f(interval[1]) < 0:
                bisect_ests.append(self.bisection(
                    f, interval[0], interval[1], 0.1))
        ...
        if df == None:
            ...
            for est in bisect_ests:
                roots.append(self.newton_raphson(
                    f, est, df, tolerance=tolerance))

        return roots

    def bisection(self, func, xL, xR, tolerance=1e-12):
        """
        ...
        if func(xL)*func(xR) < 0:  # make sure the interval has f(x) crossing the y-axis
            ...
            # use the midpoint of xL and xR instead of xM (since
            return (xL+xR)/2
    ...
    def __divide_intvl(self, xL, xR, n):
        """
